...
async function iniciarBot() {
  ...
  const sock = makeWASocket({
    ...
    auth: state,
    browser: ["sabrina bot", "Desktop", "1.0.0"],
...
// =========================================
      ...
      if (
        ...
        try {
          ...
          if (
            ...
            // Configuración que mantiene el formato del video (FULL) sin recortarlo a cuadrado
            let stickerOptions = {
              pack: "Sabrina",
              author: "by shan",
            ...
            // Ajuste silencioso de compresión secundaria si se pasa de 1 MB
          ...
          // Libera automáticamente la memoria temporal si ocurrió un fallo
          borrarTemporalesOcultos();
        }
      }
      ...
      if ((text === "!cerrar" || text === "!abrir") && isGroup) {
        try {
          ...
          // 2. Verificar si el BOT es administrador antes de cualquier otra cosa
          ...
          if (!isBotAdmin) {
            return await responder(
              "❌ Error: Asegúrate de que el bot sea administrador del grupo.",
            );
          }

          // 3. Si el bot SÍ es admin, ahora sí verificamos el estado
          ...
          if (text === "!abrir") {
            ...
            await responder(
              "🔓 *El grupo ha sido ABIERTO.* Todos los miembros pueden enviar mensajes.",
            );
          }
        } catch (e) {
